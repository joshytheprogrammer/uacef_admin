<template>
  <section>
    ...
    <b-table
      ...
      <b-table-column field="image" label="Preview" width="150" v-slot="props">
      ...
      <b-table-column field="actions" label="Actions" centered v-slot="props">
        <div class="actions">
          <span>
            <a title="view Image" :href="props.row.image">View</a>
          </span>
          <span>
...
export default {
  ...
  async fetch() {
    try {
      this.loading = true
      await this.$fire.firestore.collection('gallery').get().then((querySnapshot) => {
        querySnapshot.forEach((doc) => {
          this.data.push({id: doc.id, image: doc.data().thumbnail, active: doc.data().active})
        })
        this.loading = false
      })
    } catch (error) {
